...
(() => {
  ...
  P.drawCourse=function(){for(const o of this.objects){const z=this.objectZ(o);if(z<-88||z>12)continue;if(o.type==='gate'&&!o.processed)this.drawGate(o,z);else if(o.type==='moon'&&!o.processed)this.drawMoon(o,z);else if(o.type==='powerup'&&!o.processed)this.drawPowerup(o,z);else if(o.type==='obstacle'&&!o.processed)this.drawObstacle(o,z);else if(o.type==='enemy'&&(o===this.battleEnemy||!o.processed))this.drawEnemy(o,o===this.battleEnemy?o.battleZ:z);else if(o.type==='finish'&&!o.processed)this.drawFinishGate(z);}const density=clamp((this.visualCount-120)/300,0,1),crowdScale=lerp(1,.82,density);this.crowdBatch.draw(Math.max(1,Math.round(this.visualCount)),this.playerX,this.playerZ,COLORS.blue,this.time,{scale:crowdScale});};
  P.drawGate=function(o,z){const r=this.renderer,m=this.meshes,drawPanel=(x,opt)=>{const good=isGoodGate(opt),c=good?COLORS.teal:COLORS.red;r.draw(m.box,compose(x,1.48,z,0,0,0,4.82,2.55,.28),c);r.draw(m.box,compose(x-2.38,1.42,z+.04,0,0,0,.13,3.05,.17),good?COLORS.purple:[.42,.08,.15]);r.draw(m.box,compose(x+2.38,1.42,z+.04,0,0,0,.13,3.05,.17),good?COLORS.purple:[.42,.08,.15]);this.addWorldLabel([x,1.55,z+.22],gateLabel(opt),good?'good':'bad');};drawPanel(-2.55,o.left);drawPanel(2.55,o.right);r.draw(m.box,compose(0,1.42,z+.06,0,0,0,.13,3.05,.20),COLORS.purple);};
